...
export class PeriodoLetivoService {
  private periodoLetivoCollection;

  constructor(@Inject('DATABASE_CONNECTION') private db: Db) {
    this.periodoLetivoCollection = this.db.collection('periodo_letivo');
  }
    ...
    return new Promise((resolve, reject) => {
      stream
        ...
        .on('end', async () => {
          if (results.length > 0) {
            try {
              await this.periodoLetivoCollection.insertMany(results);
              resolve({ message: `${results.length} períodos letivos inseridos com sucesso!` });
            } catch (error) {
